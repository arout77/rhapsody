...
class Member_Controller extends Base_Controller
{
	...
	public function edit()
	{
		...
		$img_gallery = $this->plugin('image')->get_images();
		...
		{
			$upload = $this->plugin('image')->upload();
	...
	{
		if (empty($this->route->parameter[1]) || $this->route->parameter[1] === $this->session->get('username') || !isset($this->route->parameter[1]) || $this->route->parameter[1] == 'edit')
		{
			$this->edit();
		}
		else
	...
	public function view()
	{
		...
		else
		{
			...
			// register subscriber
			$subscriber = new \App\Subscribers\PostSubscriber();
			$dispatcher->addSubscriber($subscriber);
			
			// dispatch
			$dispatcher->dispatch(\App\Events\NotifyUserProfileViewedEvent::NAME, 
			new \App\Events\NotifyUserProfileViewedEvent($this->core, $data));
			
			// $this->model('Member')->member_view($data['member_id'], $this->session->get('member_id'));
			foreach ($this->model('Member')->profile_data($data['username']) as $profile)
			{
				$age = $this->plugin('formatter')->age($profile['dob']);
			}
	...
	public function all()
	{
		...
		$limit = $this->route->parameter[1];

		$query         = "SELECT * FROM users WHERE hidden = 0";
		$data['pager'] = $this->plugin('pagination');
		$data['pager']->config($query, $this->route->parameter[1], 20);
